...
public class Cliente_Ejemplo_Multi_Threaded implements Runnable{
    public String nombre_objeto_remoto = "Ejemplo_II";
    public String server;

    public Cliente_Ejemplo_Multi_Threaded (String server) {
        //Almacenamos el nombre del host del servidor
        this.server = server;
    }
    ...
    public void run() {
        ...
        try {
            ...
            Ejemplo_II instancia_local = (Ejemplo_II) registry.lookup(nombre_objeto_remoto);
            System.out.println("Invocando objeto remoto");
            instancia_local.escribir_mensaje(Thread.currentThread().getName());
        } catch (Exception e) {
            System.err.println("Ejemplo_II exception:");
            e.printStackTrace();
        }
    }
